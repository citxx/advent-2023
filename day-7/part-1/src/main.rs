...
impl Hand {
    fn from_string(s: &str) -> Option<Self> {
        ...
        if cards_opt.iter().all(|c| c.is_some()) {
            ...
            Some(Hand {
                cards,
                groups: sorted(group_counter.values()).rev().cloned().collect(),
            })
        } else {
            None
        }
